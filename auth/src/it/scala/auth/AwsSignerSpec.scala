...
class AuthSignerSpec extends IntegrationSpec with Http4sClientDsl[IO] {

  "AuthSigner" should {
    "sign request valid for S3" in withHttpClient { client =>
  ...
  def withHttpClient[A](f: Client[IO] => A) = {
    Http1Client
      .stream[IO]()
      .map(f)
      .compile
