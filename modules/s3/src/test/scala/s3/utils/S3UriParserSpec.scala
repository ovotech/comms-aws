...
class S3UriParserSpec extends UnitSpec with AsyncIOSpec {
  ...
  "S3UriParser" should {
    ...
    "parse bucket name from authority" in {
      ...
      S3UriParser.getBucket[IO](testCase).asserting(_ shouldBe Bucket("some-bucket"))
    }
    ...
    "return fail if authority empty" in {
      val testCase = "s3://"
      S3UriParser.getBucket[IO](testCase).attempt.asserting(_ shouldBe a[Left[_, _]])
    }
    ...
    "Parse key from s3 format uri" in {
      val testCase = "s3://some-bucket/object-key"
      S3UriParser.getKey[IO](testCase).asserting(_ shouldBe Key("object-key"))
    }

    "Return None from s3 format uri with no key" in {
      val testCase = "s3://some-bucket"
      S3UriParser.getKey[IO](testCase).attempt.asserting(_ shouldBe a[Left[_, _]])
    }
    ...
    "Parse multipart key from uri" in {
      ...
      S3UriParser.getKey[IO](testCase).asserting(_ shouldBe Key("key-part-1/key-part-2/key-part-3"))
    }
